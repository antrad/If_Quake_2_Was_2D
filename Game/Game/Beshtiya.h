#include "..\..\Engine\Engine\Advanced2D.h"
using namespace Advanced2D;

#define ENEMY 300
#define BESHTIYA 302
#define FLYER 0
#define FLIPPER 1
#define PLAYER 2

#define TEXTURES_ENEMY 1
#define TEXTURES_PLAYER 2

#define BIOMECH_ROCKET 2199
#define BESHTIYA_LASER 2200
#define BIOMECH_LASER 2158

#define BACKGROUND_0 0
#define BACKGROUND_1 1 
#define WORLD_BACK_0 2
#define WORLD_BACK_1 3
#define ENEMIES 4
#define PLAYER 5
#define PARTICLES_COLL 6
#define PARTICLES_NON 7
#define WORLD_FRONT_COLL 8
#define WORLD_FRONT_NON 9
#define SHADERSPRITES 10

#pragma once

class BeshtiyaModel
{
public:
	BeshtiyaModel();

	bool Load(std::string root_directory,std::string name);
	std::string root_directory;
	std::string name;

	Skeleton *skeleton;	

	Animation animation_fly;
	Animation animation_swim;
	Animation animation_attack;
};

class Beshtiya
{
private:
	double velocity_walk;
	double velocity_run;

	double energy_male;
	double energy_female;

public:
	Beshtiya(void);	

	Texture *laser_beam;	

	float laser_angle;
	float laser_angle_f;
	float laser_angle_step;

	std::vector <BeshtiyaModel*> beshtiya_models;
	
	bool LoadTextures();	
	
	Sprite* addBeshtiya(double x, double y, int id,double scale_x,double scale_y,int energy,int difficulty,std::string command);
	void addSkeleton(Sprite *a,double scale_x,double scale_y);	

	void Update(Sprite *b, Sprite *player, Sprite *player2);
	void UpdateSkeleton(Sprite *a, Sprite *player, Sprite *player2);
	void UpdateSkeleton_lite(Sprite *a);

	void HitPlatform(Sprite *a, int direction,Sprite *b);
	void TurnLeft(Sprite *b);
	void TurnRight(Sprite *b);	

	void FireFlyer(float x, float y, float angle);

};//Beshtiya
